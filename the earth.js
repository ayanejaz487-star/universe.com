document.addEventListener("DOMContentLoaded", function () {
    ...
    if (signUpBtn) {
        signUpBtn.addEventListener("click", function () {
            ...
            while (!valid) {
                // 1. Username Validation (Min 3 chars, can be more)
                username = prompt("Enter username (At least 3 characters):")?.trim();
                if (username === null) break; // User clicked cancel
                if (username.length < 3) {
                    alert("❌ Error: Username must be 3 or more characters.");
                    continue;
                }

                // 2. Email Validation (Min 8 before domain, can be more)
                ...
                // Check if they typed the whole thing or just the prefix
                ...
                if (prefix.length < 8 || !isValidEmail(email)) {
                    alert("❌ Error: Email prefix must be at least 8 characters long (e.g., student123@umail.com).");
                    continue;
                }
                ...
                if (!isStrongPassword(password)) {
                    alert("❌ Error: Password is too weak! Ensure it has 8+ chars with numbers, letters, and symbols.");
                    continue;
                }

                // SUCCESS: Save data and exit loop
                valid = true;
